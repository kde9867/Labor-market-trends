...
# AI 관련 직무 필터링 (우선순위 기반 중복 제거)
def filter_ai_jobs(metadata: pd.DataFrame, embeddings: np.ndarray, ai_keywords: list = None) -> Tuple[pd.DataFrame, np.ndarray]:
    if ai_keywords is None:
        # 우선순위: ChatGPT > LLM > ML > 딥러닝 > 인공지능 > AI > SW > 로보틱스
        ai_keywords = ['ChatGPT', 'LLM', 'ML', '딥러닝', '인공지능', 'AI', 'SW', '로보틱스']
    ...
    for i, keyword in enumerate(ai_keywords):
        if i > 0:         
            current_mask = keyword_masks[keyword].copy()
            current_mask[list(excluded_indices)] = False
            keyword_masks[keyword] = current_mask

        matched_indices = keyword_masks[keyword][keyword_masks[keyword]].index.tolist()
        excluded_indices.update(matched_indices)
    
    # 전체 AI 관련 문서 마스크 (중복 제거 후)
    final_mask = pd.Series([False] * len(metadata))
    for mask in keyword_masks.values():
        final_mask |= mask
    
    md_ai = metadata[final_mask].reset_index(drop=True)
    emb_ai = embeddings[final_mask.values]
    
    print(f"AI 직무 필터링 (중복 제거): {len(md_ai):,}개 ({len(md_ai)/len(metadata)*100:.1f}%)")
    print("키워드별 문서 수 (우선순위 적용 후):")
    for keyword in ai_keywords:
        count = keyword_masks[keyword].sum()
        if count > 0:
...
# 시간 기간 설정
def add_time_periods(metadata: pd.DataFrame, date_col: str, period_months: int = 24) -> pd.DataFrame:
    md = metadata.copy()
...
def extract_representative_docs(metadata: pd.DataFrame, embeddings_2d: np.ndarray, centroids_df: pd.DataFrame, n_docs: int = 5) -> Dict:
...
# 시각화 (라벨 겹침 개선)
def plot_centroid_movement(ax, centroids_df: pd.DataFrame, title: str, color_main: str = "darkblue", label_offset_scale: float = 1.5):
    plt.rcParams['axes.unicode_minus'] = False
...
# 대표 문서 출력
def print_statistics(centroids_df: pd.DataFrame, title: str, representative_docs: Dict = None):
