...
def preprocess_work24(input_path: str, output_path = 'work24_preprocessed.csv') -> pd.DataFrame:
    ...
    valid_posting = df['등록일자_normalized'].notna().sum()
    valid_expiration = df['마감일자_normalized'].notna().sum()
...
def preprocess_saramin(input_path: str, output_path: str = 'saramin_preprocessed.csv') -> pd.DataFrame:
    df = pd.read_csv(input_path)
    print(f"  원본: {len(df):,}개, {len(df.columns)}개 컬럼")
    ...
    valid_posting = df['posting_date_normalized'].notna().sum()
    valid_expiration = df['expiration_date_normalized'].notna().sum()
...
def preprocess_wanted(input_path: str, output_path: str = 'wanted_preprocessed.csv') -> pd.DataFrame:
    df = pd.read_csv(input_path)
    print(f"  원본: {len(df):,}개, {len(df.columns)}개 컬럼")
...
def preprocess_all_platforms(
    ...
    saramin_path: str,
    wanted_path: str,
    work24_output: str = '/path/Labor-market-trends/datasets/work24_preprocessed.csv',
    saramin_output: str = '/path/Labor-market-trends/datasets/saramin_preprocessed.csv',
    wanted_output: str = '/path/Labor-market-trends/datasets/wanted_preprocessed.csv'
):

    work24_df = preprocess_work24(work24_path, work24_output)
    saramin_df = preprocess_saramin(saramin_path, saramin_output)
    wanted_df = preprocess_wanted(wanted_path, wanted_output)
    
    print("\n" + "=" * 70)
    print("전처리 완료")
    print("=" * 70)
    print(f"Work24:  {len(work24_df):,}개 → {work24_output}")
    print(f"Saramin: {len(saramin_df):,}개 → {saramin_output}")
    print(f"Wanted:  {len(wanted_df):,}개 → {wanted_output}")
...
if __name__ == "__main__":
    preprocess_all_platforms(
        work24_path='/home/jovyan/feature_reasoning/doeun/AI_job/work24.csv',
        saramin_path='/home/jovyan/feature_reasoning/doeun/AI_job/saramin.csv',
        wanted_path='/home/jovyan/feature_reasoning/doeun/AI_job/wanted.csv'
    )