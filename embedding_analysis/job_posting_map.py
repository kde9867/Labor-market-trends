"""
Job Posting Embedding Analysis Module (Simplified)
"""
import pickle
import numpy as np
import pandas as pd
...
def reduce_to_2d(embeddings: np.ndarray, n_neighbors: int = 15, min_dist: float = 0.1) -> np.ndarray:
    """UMAP 2D 차원 축소"""
...
def compute_kde(X: np.ndarray, Y: np.ndarray, axis_range: tuple, 
                grid_size: int = 200, bw_adjust: float = 0.5):
    """KDE 밀도 계산"""
...
def plot_kde_by_year(metadata: pd.DataFrame, coords: np.ndarray, 
                     platform: str = "wanted", bins: list = None, labels: list = None,
                     date_col: str = None):
    """연도별 KDE 플롯"""
    if bins is None:
